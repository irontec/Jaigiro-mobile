...
- (id)initWithFrame:(CGRect)frame
{
    ...
    self.backgroundColor = [UIColor clearColor];//Fondo del calendario
...
- (void)drawRect:(CGRect)rect
{
    ...
    dayTitles = [DateHelper getDaysArray];
    monthTitles = [DateHelper getMonthsArray];
    
    CGContextRef context = UIGraphicsGetCurrentContext();

    CGColorRef shadow2 = [UIColor blackColor].CGColor;//Sombra del nombre del mes y de los dias de la semana
    ...
    for (NSInteger i = 0; i < dayTitles.count; i++)
    {
        ...
        CGRect dayHeaderFrame = CGRectMake(floor(i * hDiff) - 1
                                           ...
                                           , hDiff
                                           , 30);
        [[UIColor whiteColor] setFill];//Color de los nombres de la semana
    ...
    CGRect textFrame = CGRectMake(0
                                  ...
                                  , width
                                  , headerHeight);
    [[UIColor whiteColor] setFill];//Color del nombre del mes
    ...
    [[UIColor whiteColor] setFill];//Color flecha izquierda de seleccion del mes
    [backArrowPath fill];
    ...
    [[UIColor whiteColor] setFill];//Color flecha derecha de seleccion del mes
...
- (void) periodUpdated
{
    ...
    NSInteger maxNumberOfCells    = ceil((CGFloat)numDaysInMonth / 7.0f) * 7 - 1;
...
- (NSDate *) dateForPoint: (CGPoint)point
{
    ...
    monthStartDay           = (monthStartDay + (self.mondayFirstDayOfWeek?5:6)) % 7;
    numDaysInMonth         += monthStartDay;
    NSInteger maxNumberOfRows     = ceil((CGFloat)numDaysInMonth / 7.0f) - 1;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    self.backgroundColor = [UIColor clearColor];//Color de fondo del calendario
    self.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat vDiff  = (height - headerHeight) / 7;
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGColorRef shadow2 = [UIColor blackColor].CGColor;//Color de sombra de los dias del mes
    CGSize shadow2Offset = CGSizeMake(1, 1);
    CGFloat shadow2BlurRadius = 1;
...
//        CGContextFillRect(context, rect);                                                    // /      debug
	...
	NSInteger weekdayOfFirst = ([dateOnFirst weekday] + (_mondayFirstDayOfWeek?5:6)) % 7 + 1;
    ...
    for(NSInteger i = 0; i <= weekdayOfFirst-2; i++) {
        ...
        UIColor *color = [UIColor colorWithWhite:0.6f alpha:1.0f];//Color de los dias del mes anterior
	...
	for (NSInteger i = 0; i < 6; i++)
    {
		for(NSInteger j = 0; j < 7; j++)
        {
			...
			if(dayNumber >= (weekdayOfFirst-1) && day <= numDaysInMonth) {
                ...
                {
                    color = [UIColor colorWithRed: 0.98 green: 0.24 blue: 0.09 alpha: 1];//Color del dia actual del mes
                    color = [UIColor whiteColor];
                }
                else 
                {
                    color = [UIColor whiteColor];//Color de todos los dias del mes, excepto el actual
                }
    ...
    //Find number of days in previous month
    NSInteger weekdayOfNextFirst = (weekdayOfFirst - 1 + numDaysInMonth) % 7;
    
    if( weekdayOfNextFirst > 0 )
    {
        //Draw the text for each of those days.
        for ( NSInteger i = weekdayOfNextFirst; i < 7; i++ )
        {
            NSInteger day = i - weekdayOfNextFirst + 1;
            NSString *string = [NSString stringWithFormat:@"%d", day];
            CGRect dayHeader2Frame = CGRectMake(floor(i * hDiff) - 1
                                                ...
                                                , hDiff, self.font.pointSize); 
            UIColor *color = [UIColor colorWithWhite:0.6f alpha:1.0f];//Color de los dias del siguiente mes
            drawString( string, dayHeader2Frame, color );
        }
    }
}
