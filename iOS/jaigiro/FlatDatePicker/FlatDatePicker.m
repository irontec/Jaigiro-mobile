...
//        [self buildSelectorMonthsOffsetX:(_scollViewDays.frame.size.width + kFlatDatePickerScrollViewLeftMargin) andWidth:kFlatDatePickerScrollViewMonthWidth];
//        [self buildSelectorDaysOffsetX:(_scollViewMonths.frame.origin.x + _scollViewMonths.frame.size.width + kFlatDatePickerScrollViewLeftMargin) andWidth:(self.frame.size.width - (_scollViewMonths.frame.origin.x + _scollViewMonths.frame.size.width + kFlatDatePickerScrollViewLeftMargin))];
...
//        [self buildSelectorSecondsOffsetX:((self.frame.size.width / 3.0) * 2.0) andWidth:(self.frame.size.width / 3.0)];
        ...
        [self buildSelectorSecondsOffsetX:0.0 andWidth:0.0];
    ...
    //Cambiar columnas
    CGRect temp;
    temp = _scollViewYears.frame;
    _scollViewYears.frame = _scollViewDays.frame;
    _scollViewDays.frame = temp;
}
...
- (void)buildSelectorLabelsDays {
    ...
    CGFloat offsetContentScrollView = (_scollViewDays.frame.size.height - kFlatDatePickerScrollViewItemHeight) / 2.0;//dias
...
- (void)buildSelectorLabelsMonths {
    ...
    CGFloat offsetContentScrollView = (_scollViewDays.frame.size.height - kFlatDatePickerScrollViewItemHeight) / 2.0;//meses
...
- (void)buildSelectorLabelsYears {
    ...
    CGFloat offsetContentScrollView = (_scollViewYears.frame.size.height - kFlatDatePickerScrollViewItemHeight) / 2.0;//años
    ...
    for (NSInteger i = 0; i < _years.count; i++) {
        ...
        NSString *year = (NSString*)[_years objectAtIndex:i];
        
        UILabel *labelYear = [[UILabel alloc] initWithFrame:CGRectMake(0.0, (i * kFlatDatePickerScrollViewItemHeight) + offsetContentScrollView, _scollViewYears.frame.size.width, kFlatDatePickerScrollViewItemHeight)];
        labelYear.text = year;
        labelYear.font = kFlatDatePickerFontLabel;
        labelYear.textAlignment = NSTextAlignmentCenter;
        labelYear.textColor = kFlatDatePickerFontColorLabel;
        labelYear.backgroundColor = [UIColor clearColor];
        
        [_labelsYears addObject:labelYear];
        [_scollViewYears addSubview:labelYear];
    }
...
- (void)actionButtonValid {
    ...
    if (self.delegate != nil && [self.delegate respondsToSelector:@selector(flatDatePicker:didValid:date:)]) {
        ...
        date = [self getDate];
        [self.delegate flatDatePicker:self didValid:_buttonValid date:date];
    }
}
...
- (NSMutableArray*)getMonths {
    NSMutableArray *meses = [[NSMutableArray alloc] initWithObjects:@"Urtarrila", @"Otsaila", @"Martxoa", @"Apirila", @"Maiatza", @"Ekaina", @"Uztaila", @"Abuztua", @"Iraila", @"Urria", @"Azaroa", @"Abendua", nil];
    return meses;
}
...
- (NSDate*)convertToDateDay:(int)day month:(int)month year:(int)year hours:(int)hours minutes:(int)minutes seconds:(int)seconds date:(NSDate*)date; {
    ...
    if (self.datePickerMode == FlatDatePickerModeTime) {
        ...
        dateComponents = [self.calendar components:NSYearCalendarUnit|NSMonthCalendarUnit|NSDayCalendarUnit|NSHourCalendarUnit|NSMinuteCalendarUnit fromDate:[NSDate date]];
        [dateComponents setTimeZone:self.timeZone];
    ...
    NSString *str =[NSString stringWithFormat:@"%d/%d/%d %d:%d", day, month, year, hours, minutes];
    NSDateFormatter *formatter = [[NSDateFormatter alloc]init];
    [formatter setDateFormat:@"dd/MM/yyyy HH:mm"];
    NSDate *dat = [[NSDate alloc]init];
    dat = [formatter dateFromString:str];
//    [dateComponents setYear:+1700];
    //dat =  [[NSCalendar currentCalendar] dateByAddingComponents:dateComponents toDate:dat  options:0];
